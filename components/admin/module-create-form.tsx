...
import { createModule } from '@/app/admin/actions';
...
export const ModuleCreateForm = ({ courses }: ModuleCreateFormProps) => {
  const { t } = useLanguage();
  ...
  return (
    <form action={formAction} className="card-surface flex flex-col gap-4 p-6">
      <header>
        <h3 className="text-2xl font-semibold text-white">{t('admin.createModule')}</h3>
      </header>
      <label className="flex flex-col gap-2 text-sm text-white/80 md:w-72">
