...
create index if not exists lesson_exercises_lesson_idx on public.lesson_exercises(lesson_id);

alter table public.lesson_exercises enable row level security;

create policy "Lesson exercises readable by anyone" on public.lesson_exercises
  for select using (true);

-- Lesson external links
create table if not exists public.lesson_external_links (
...
create index if not exists lesson_external_links_lesson_idx on public.lesson_external_links(lesson_id);

alter table public.lesson_external_links enable row level security;

create policy "Lesson external links readable by anyone" on public.lesson_external_links
  for select using (true);
...
grant select on public.courses, public.modules, public.lessons, public.lesson_exercises, public.lesson_external_links to anon, authenticated, service_role;
